...
export async function getAllArticles(lang) {
  try {
    ...
    let blogArticles = articles.filter((article) => article.type === 'blog');
    return { success: true, articles: JSON.parse(JSON.stringify(blogArticles)) };
  } catch (error) {
...
export async function getAllBlogsSlug(id) {
  try {
    await connectToDB();
    const articles = await Article.find({ type: 'blog' }, 'slug lang').lean();
    return { success: true, articles };
  } catch (error) {
    console.error('Error fetching all blog slugs:', error.message);
    return { success: false, error: error.message };
  }
}
